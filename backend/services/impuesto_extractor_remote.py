...
class ImpuestoExtractor(PDFExtractor):
    ...
    def _detect_expediente(self, text: str) -> str:
        ...
        pattern = r'Expediente/Referencia.*?:?\s*([A-Z0-9]+)'
    ...
    def _extract_detalle_liquidacion(self, text: str) -> list:
        """Extrae la tabla de liquidación de intereses con cuotas de pago"""
        try:
            # Estrategia final y definitiva:
            # 1. Buscar "ANEXO II" usando búsqueda de texto plano (find) para evitar problemas de regex
            # 2. Si se encuentra, cortar el texto desde ahí
            # 3. Buscar la tabla en el texto cortado
            
            idx_anexo = text.upper().find("ANEXO II")
            if idx_anexo != -1:
                logger.info(f"📄 'ANEXO II' encontrado en posición {idx_anexo}. Analizando desde ahí.")
                texto_anexo = text[idx_anexo:]
                
                # Buscar zona de detalle DENTRO del anexo
                detalle_match = re.search(
                    r'(?:DETALLE DE LA LIQUIDACIÓN|Número Liquidaci[óo]n)(.+?)(?:TOTAL|$)', 
                    texto_anexo, 
                    re.DOTALL | re.IGNORECASE
                )
                
                if detalle_match:
                    detalle_text = detalle_match.group(1)
                else:
                    # Fallback: Usar todo el texto del anexo si no se encuentra delimitador claro
                    detalle_text = texto_anexo
                    logger.warning("No se encontró 'DETALLE' en Anexo II, usando todo el texto del anexo")
            else:
                logger.warning("❌ No se encontró 'ANEXO II'. Buscando 'DETALLE' en todo el texto.")
                # Fallback peligroso (puede coger texto de pg 1), pero mejor que nada
                detalle_match = re.search(
                    r'DETALLE DE LA LIQUIDACIÓN(.+?)(?:TOTAL|$)', 
                    text, 
                    re.DOTALL | re.IGNORECASE
                )
                if detalle_match:
                    ...
                    detalle_text = text[-5000:]
            
            # Limpiar el texto para depuración
            if len(detalle_text) > 1000:
                logger.debug(f"Texto a analizar (inicio): {detalle_text[:200]}...")
            
            # PARSER DE MÁQUINA DE ESTADOS - IGUAL QUE ANTES PERO CON ZONA CORRECTA 
            
            # PARSER DE MÁQUINA DE ESTADOS
            # Más robusto que regex pura para tablas verticales con saltos de línea variables
            lines = detalle_text.split('\n')
            cuotas = []
            current_cuota = {}
            state = 0 
            # Estados:
            # 0: Buscando referencia (A08...)
            # 1: Buscando Importe + Fecha + Fecha
            # 2: Buscando Días
            # 3: Buscando Tipo Interés
            # 4: Buscando Intereses
            # 5: Buscando Total Intereses
            
            for line in lines:
                line = line.strip()
                if not line: continue
                
                # Estado 0: Buscar referencia A...
                if state == 0:
                    if re.match(r'[A-Z0-9]{15,}', line):
                        current_cuota = {'numero_liquidacion': line}
                        state = 1
                
                # Estado 1: Importe Fecha Fecha (ej: 1.291,52 21-10-2025 20-12-2025)
                elif state == 1:
                    # Regex tolerante a espacios
                    ...
                    if match:
                        ...
                        state = 2
                    # Si encontramos otra referencia, reiniciamos (recuperación de error)
                    elif re.match(r'[A-Z0-9]{15,}', line):
                         current_cuota = {'numero_liquidacion': line}
                         state = 1

                # Estado 2: Días (entero)
                elif state == 2:
                    if line.isdigit():
                        current_cuota['dias'] = int(line)
                        state = 3
                
                # Estado 3: Tipo interés (ej: 4.062)
                elif state == 3:
                     # A veces viene como "4.062" o "4,062"
                    if re.match(r'^[\d,.]+$', line):
                        current_cuota['tipo_interes'] = self._parse_amount(line)
                        state = 4
                
                # Estado 4: Intereses (ej: 8,77)
                elif state == 4:
                    if re.match(r'^[\d,.]+$', line):
                        current_cuota['intereses'] = self._parse_amount(line)
                        state = 5
                
                # Estado 5: Total intereses (ej: 8,77)
                elif state == 5:
                    if re.match(r'^[\d,.]+$', line):
                        current_cuota['total_intereses'] = self._parse_amount(line)
                        # ¡Completado!
                        cuotas.append(current_cuota)
                        state = 0 # Volver a buscar
            ...
            for i, line in enumerate(lines):
                ...
                if state == 0:
                    # Buscar referencia A08... tolerando prefijos como ": " o "Número Liquidación: "
                    match_ref = re.search(r'([A-Z0-9]{15,})', line)
                    if match_ref:
                        ref = match_ref.group(1)
                        # Ignorar si es el NIF de la cabecera (B66...) o expediente
                        if len(ref) > 15 and not ref.startswith('B66'): 
                            current_cuota = {'numero_liquidacion': ref}
                            state = 1
                
                elif state == 1:
                    # Buscar patrón de importes y fechas: 1.291,52 21-10-2025 20-12-2025
                    match = re.search(r'([\d,.]+)\s+(\d{2}-\d{2}-\d{4})\s+(\d{2}-\d{2}-\d{4})', line)
                    if match:
                        current_cuota['importe_principal'] = self._parse_amount(match.group(1))
                        current_cuota['fecha_desde'] = match.group(2)
                        current_cuota['fecha_hasta'] = match.group(3)
                        state = 2
                    # Si encontramos SOLO la referencia de nuevo sin haber completado la cuota, reiniciamos
                    elif re.search(r'([A-Z0-9]{15,})', line):
                            ref = re.search(r'([A-Z0-9]{15,})', line).group(1)
                            if len(ref) > 15:
                                current_cuota = {'numero_liquidacion': ref}
                                state = 1
                    ...
                    if line.isdigit():
                        ...
                        if re.match(r'^[\d,.]+$', line):
                            # Para el tipo de interés, el formato es confuso (4.062).
                            # Si aplicamos la lógica estándar (punto=miles), queda 4062.
                            # Hacemos un parseo especial: si tiene punto y no coma, y es < 20 caracteres, asumimos punto decimal.
                            val_str = line
