    ...
    def upload_contratos():
        ...
        for file in files:
            ...
            try:
                ...
                for res in resultados:
                    ...
                    if empresa_id:
                        ...
                        if not doc_existente:
                            ...
                            db.session.flush() # Obtener doc.id antes del commit total
                ...
                # Agrupación post-commit para que todos los documentos nuevos tengan ID y sean consultables
                from utils.document_utils import auto_group_contratos
                for detalle in processed_details:
                    if detalle.get('doc_id') and detalle.get('estado') == 'exito':
                        try:
                            auto_group_contratos(detalle['doc_id'], gestoria_id, current_user.id)
                        except Exception as e:
                            current_app.logger.error(f"Error agrupando contrato {detalle['doc_id']}: {e}")
